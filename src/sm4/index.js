...
function sm4(inArray, key, cryptFlag, {
  padding = 'default', mode, iv, output = 'string'
} = {}) {
  // 检查 key
  ...
  // 新增填充，原项目采用pkcs#5，为兼容AOB，修改为此种方式
  if (padding === 'default' && cryptFlag !== DECRYPT) {
    const paddingCount = BLOCK - inArray.length % BLOCK
    let inArrayHex = ArrayToHex(inArray) + '80'
    for (let i = 0; i < paddingCount; i++) inArrayHex += '00'
    inArray = hexToArray(inArrayHex)
  }
  ...
  // 去除填充
  if (padding === 'default' && cryptFlag === DECRYPT) {
    let outArrayHex = ArrayToHex(outArray)
    const pos = outArrayHex.lastIndexOf('80')
    outArrayHex = outArrayHex.slice(0, pos)
    outArray = hexToArray(outArrayHex)
  }
